...
const RecommendedProperties = () => {
  ...
  const getBHKString = (property) => {
    const bhks = [];
    if (property.Beds_2bhk) bhks.push("2 BHK");
    if (property.Beds_3bhk) bhks.push("3 BHK");
    if (property.Beds_4bhk) bhks.push("4 BHK");
    if (property.Beds_5bhk) bhks.push("5 BHK");
    if (property.Penthouse) bhks.push("Penthouse");
    return bhks.join(", ");
  };
          ...
          {visibleCards.map((property, i) => (
            <div className="property-card" key={i}>
              <div className='property-card-image'>
                <img src={
                      property.Main_Image
                        ? `${BASE_URL}/uploads/${property.Developer_Logo}`
                        : "../img/elaanlogo.png"
                    } alt={property.title} />
              </div>
